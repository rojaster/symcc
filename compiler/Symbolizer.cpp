...
void Symbolizer::symbolizeFunctionArguments(Function& F) {
    ...
    for (auto& arg : F.args()) {
        if (!arg.user_empty())
            symbolicExpressions[&arg] = IRB.CreateCall(
                runtime.getParameterExpression, IRB.getInt8(arg.getArgNo()));
    }
}
...
void Symbolizer::finalizePHINodes() {
    ...
    for (auto* phi : phiNodes) {
        ...
        // A PHI node that receives only compile-time constants can be replaced
...
void Symbolizer::shortCircuitExpressionUses() {
    for (auto& symbolicComputation : expressionUses) {
        ...
        // symbolic computation. Otherwise, we need to check all input
        // expressions and create an output expression.
        ...
        auto numUnknownConcreteness = std::count_if(
            symbolicComputation.inputs.begin(),
            symbolicComputation.inputs.end(), [&](const Input& input) {
                return (input.getSymbolicOperand() != nullExpression);
            });
        for (unsigned argIndex = 0;
             argIndex < symbolicComputation.inputs.size(); argIndex++) {
            ...
            // We only need a run-time check for concreteness if the argument
            // isn't known to be concrete at compile time already. However,
            // there is one exception: if the computation only has a single
            // argument of unknown concreteness, then we know that it must be
            // symbolic since we ended up in the slow path. Therefore, we can
            // skip expression generation in that case.
            ...
            if (needRuntimeCheck) {
                ...
                argPHI->addIncoming(newArgExpression,
                                    newArgExpression->getParent());
        ...
        // if we've taken the fast path and the symbolic expression computed
        // above if short-circuiting wasn't possible.
...
void Symbolizer::handleIntrinsicCall(CallBase& I) {
    ...
    case Intrinsic::memcpy: {
        ...
        // The intrinsic allows both 32 and 64-bit integers to specify the
        // length; convert to the right type if necessary. This may truncate the
        // value on 32-bit architectures. However, what's the point of
        // specifying a length to memcpy that is larger than your address space?
    ...
    case Intrinsic::fabs: {
        ...
        IRBuilder<> IRB(&I);
        auto abs =
            buildRuntimeCall(IRB, runtime.buildFloatAbs, I.getOperand(0));
    ...
    case Intrinsic::ctlz: {
        ...
        llvm::errs()
            << "Warning: losing track of symbolic expressions at bit-count "
               "operation "
            << I << "\n";
        break;
    }
    case Intrinsic::returnaddress: {
        // Obtain the return address of the current function or one of its
        // parents on the stack. We just concretize.

        llvm::errs() << "Warning: using concrete value for return address\n";
        break;
    }
    case Intrinsic::bswap: {
        ...
        IRBuilder<> IRB(&I);
        auto swapped =
            buildRuntimeCall(IRB, runtime.buildBswap, I.getOperand(0));
        registerSymbolicComputation(swapped, &I);
        break;
    }
    default:
        llvm::errs() << "Warning: unhandled LLVM intrinsic "
                     << callee->getName()
...
void Symbolizer::handleInlineAssembly(CallInst& I) {
    if (I.getType()->isVoidTy()) {
        llvm::errs() << "Warning: skipping over inline assembly " << I << '\n';
        return;
    }
...
void Symbolizer::visitBinaryOperator(BinaryOperator& I) {
    ...
    if (I.getOperand(0)->getType() == IRB.getInt1Ty()) {
        ...
        case Instruction::Xor:
            ...
            break;
        default:
            llvm::errs() << "Can't handle Boolean operator " << I << '\n';
...
void Symbolizer::visitGetElementPtrInst(GetElementPtrInst& I) {
    ...
    for (auto type_it = gep_type_begin(I), type_end = gep_type_end(I);
        ...
        if (auto* structType = type_it.getStructTypeOrNull()) {
            ...
            // multiplication ourselves instead of having the solver do it.
            // Also, if the element size is 1, we can omit the multiplication.
...
void Symbolizer::visitCastInst(CastInst& I) {
    ...
    if (opcode != Instruction::SExt && opcode != Instruction::ZExt) {
        llvm::errs() << "Warning: unhandled cast instruction " << I << '\n';
        return;
    }
    ...
    auto insert = buildRuntimeCall(
        ...
        {{I.getAggregateOperand(), true},
         ...
         {IRB.getInt64(aggregateMemberOffset(I.getAggregateOperand()->getType(),
          ...
          false},
         {IRB.getInt8(
              isLittleEndian(I.getInsertedValueOperand()->getType()) ? 1 : 0),
          false}});
...
void Symbolizer::visitSwitchInst(SwitchInst& I) {
    ...
    for (auto& caseHandle : I.cases()) {
        auto* caseTaken =
            IRB.CreateICmpEQ(condition, caseHandle.getCaseValue());
...
void Symbolizer::visitInstruction(Instruction& I) {
    ...
    llvm::errs() << "Warning: unknown instruction " << I
                 << "; the result will be concretized\n";
}
...
CallInst* Symbolizer::createValueExpression(Value* V, IRBuilder<>& IRB) {
    ...
    if (valueType->isIntegerTy()) {
        ...
        if (bits == 1) {
            ...
            // Anything up to the maximum supported 128 bits. Those integers are
            // a bit tricky because the symbolic backends don't support them per
            // se. We have a special function in the run-time library that
            // handles them, usually by assembling expressions from smaller
            // chunks.
            return IRB.CreateCall(
                runtime.buildInteger128,
                {IRB.CreateTrunc(
                     IRB.CreateLShr(V, ConstantInt::get(valueType, 64)),
                     IRB.getInt64Ty()),
...
Symbolizer::forceBuildRuntimeCall(IRBuilder<>& IRB, SymFnT function,
    ...
    for (const auto& [arg, symbolic] : args) {
        functionArgs.push_back(symbolic ? getSymbolicExpressionOrNull(arg)
                                        : arg);
    }
