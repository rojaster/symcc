...
class Symbolizer : public llvm::InstVisitor<Symbolizer> {
  public:
    ...
    /// Insert code to obtain the symbolic expressions for the function
    /// arguments.
  ...
  private:
    ...
    struct SymbolicComputation {
        ...
        llvm::SmallVector<Input, kExpectedSymbolicArgumentsPerComputation>
            inputs;
    ...
    llvm::CallInst* createValueExpression(llvm::Value* V,
                                          llvm::IRBuilder<>& IRB);
    ...
    /// corresponding symbolic expression will be passed to the run-time
    /// function. Moreover, the use of symbolic expressions will be recorded in
    /// the resulting SymbolicComputation. If all symbolic arguments are known
    /// to be concrete (e.g., because they are compile-time constants), no call
    ...
    buildRuntimeCall(llvm::IRBuilder<>& IRB, SymFnT function,
        ...
        if (std::all_of(args.begin(), args.end(),
                        [this](std::pair<llvm::Value*, bool> arg) {
                            return (getSymbolicExpression(arg.first) ==
                                    nullptr);
                        })) {
            return {};
        }
    ...
    /// pointer if the host uses larger addresses than the target. Therefore,
    /// use this function only when such loss is acceptable (e.g., when
    /// generating site identifiers to be passed to the backend, where
    /// collisions of the least significant bits are reasonably unlikely).
    ///
    ...
    /// would require modifying code that we don't control (in the case of
    /// Symcc).
    ...
    ///
    /// For pointer values, the stored value is an expression describing the
    /// value of the pointer itself (i.e., the address, not the referenced
    /// value). For structure values, the expression is a single large bit
    /// vector.
    ///
    /// TODO This member adds a lot of complexity: various methods rely on it,
    /// and finalizePHINodes invalidates it. We may want to pass the map around
    /// explicitly.
    ...
    /// loop when trying to generate symbolic expressions recursively.
    /// Therefore, we only insert a dummy symbolic expression for each PHI node
    /// and fix it after all instructions have been processed.
