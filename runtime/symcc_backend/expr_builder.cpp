...
ExprRef evaluateInDifferentType(ExprBuilder* builder, ExprRef op,
    ...
    case URem: {
        ...
        return builder->createBinaryExpr(
            op->kind(), builder->createExtract(op->getChild(0), index, bits),
...
void ExprBuilder::setNext(ExprBuilder* next) { next_ = next; }
...
ExprRef ExprBuilder::createTrue() { return createBool(true); }

ExprRef ExprBuilder::createFalse() { return createBool(false); }
...
void CacheExprBuilder::insertToCache(ExprRef e) { cache_.insert(e); }

ExprRef CacheExprBuilder::findInCache(ExprRef e) { return cache_.find(e); }
    ...
    if (auto ee_l = castAs<ExtractExpr>(l)) {
        if (auto ee_r = castAs<ExtractExpr>(r)) {
            if (ee_l->expr() == ee_r->expr() &&
                ...
                return createExtract(ee_l->expr(), ee_r->index(),
                                     ee_r->bits() + ee_l->bits());
            }
        }
    }
...
ExprRef CommonSimplifyExprBuilder::createExtract(ExprRef e, uint32_t index,
    ...
    if (auto ce = castAs<ConcatExpr>(e)) {
        ...
        if (index >= ce->getRight()->bits())
            return createExtract(ce->getLeft(), index - ce->getRight()->bits(),
                                 bits);
        ...
        if (ce->getLeft()->isConstant()) {
            return createConcat(
                createExtract(ce->getLeft(), 0,
                              bits - ce->getRight()->bits() + index),
                createExtract(ce->getRight(), index,
                              ce->getRight()->bits() - index));
        }
        ...
        if (ee->kind() == ZExt && index >= ee->expr()->bits())
    ...
    if (auto const_l = castAs<ConstantExpr>(l)) {
        if (auto concat_r = castAs<ConcatExpr>(r)) {
            ...
            if (ExprRef and_left = simplifyAnd(l_left, r_left)) {
                return createConcat(
                    and_left,
                    createAnd(createExtract(l, 0, r_right->bits()), r_right));
            }
        }
    }
    ...
    if (auto const_l = castAs<ConstantExpr>(l)) {
        if (auto concat_r = castAs<ConcatExpr>(r)) {
            ...
            if (ExprRef and_left = simplifyOr(l_left, r_left)) {
                return createConcat(
                    and_left,
                    createOr(createExtract(l, 0, r_right->bits()), r_right));
            }
        }
    }
    ...
    if (auto const_l = castAs<ConstantExpr>(l)) {
        if (auto concat_r = castAs<ConcatExpr>(r)) {
            ...
            if (ExprRef and_left = simplifyXor(l_left, r_left)) {
                return createConcat(
                    and_left,
                    createXor(createExtract(l, 0, r_right->bits()), r_right));
            }
        }
    }
...
ExprRef SymbolicExprBuilder::createConcat(ExprRef l, ExprRef r) {
    ...
    if (auto ce = castAs<ConcatExpr>(l)) {
        return createConcat(l->getLeft(), createConcat(l->getRight(), r));
    }
...
ExprRef SymbolicExprBuilder::createAdd(ConstantExprRef l,
...
ExprRef SymbolicExprBuilder::createAdd(NonConstantExprRef l,
                                       NonConstantExprRef r) {
    if (l == r) {
        // l + l ==> 2 * l
...
ExprRef SymbolicExprBuilder::createSub(ConstantExprRef l,
                                       NonConstantExprRef r) {
    switch (r->kind()) {
    case Add: {
        // C_0 - (C_1 + X) ==> (C_0 - C1) - X
...
ExprRef SymbolicExprBuilder::createSub(NonConstantExprRef l,
                                       NonConstantExprRef r) {
    // X - X ==> 0
    if (l == r)
        return createConstant(0, l->bits());
    ...
    case Add: {
        if (l->getChild(0)->isConstant()) {
            // (C + Y) - Z ==> C + (Y - Z)
            return createAdd(l->getChild(0), createSub(l->getChild(1), r));
        }
        break;
    }
    case Sub: {
        if (l->getChild(0)->isConstant()) {
            // (C - Y) - Z ==> C - (Y + Z)
            return createSub(l->getChild(0), createAdd(l->getChild(1), r));
        }
        break;
    }
    default:
...
ExprRef SymbolicExprBuilder::createMul(ConstantExprRef l,
                                       NonConstantExprRef r) {
    // C_0 * (C_1 * x) ==> (C_0 * C_1) * x
...
ExprRef SymbolicExprBuilder::createSDiv(NonConstantExprRef l,
                                        ConstantExprRef r) {
    // x /s -1 = -x
    if (r->isAllOnes())
        return createNeg(l);
    ...
    if (auto sext_l = castAs<SExtExpr>(l)) {
        ...
        if (x->bits() >= r->getActiveBits()) {
            return createSExt(createSDiv(x, createExtract(r, 0, x->bits())),
                              l->bits());
        }
    }
...
ExprRef SymbolicExprBuilder::createUDiv(NonConstantExprRef l,
                                        ConstantExprRef r) {
        ...
        if (ce_l->isZero()) {
            if (r->getActiveBits() <= ce_r->bits()) {
                ExprRef e = createConcat(
                    ce_l, createUDiv(ce_r, createExtract(r, 0, ce_r->bits())));
                return e;
            }
        }
    }
...
ExprRef SymbolicExprBuilder::createAnd(ConstantExprRef l,
                                       NonConstantExprRef r) {
    return ExprBuilder::createAnd(l, r);
}

ExprRef SymbolicExprBuilder::createAnd(NonConstantExprRef l,
                                       NonConstantExprRef r) {
    // A & A  ==> A
    if (l == r)
        return l;
    ...
    if (auto ce_l = castAs<ConcatExpr>(l)) {
        if (auto ce_r = castAs<ConcatExpr>(r)) {
            if (ce_l->getLeft()->bits() == ce_r->getLeft()->bits()) {
                ...
                return createConcat(createAnd(l->getLeft(), r->getLeft()),
    ...
    if (auto ce = castAs<ConcatExpr>(r)) {
        ...
        return createConcat(
            createOr(
                ...
                ce->getLeft()),
            createOr(createExtract(l, 0, ce->getRight()->bits()),
                     ce->getRight()));
    }
...
ExprRef SymbolicExprBuilder::createOr(NonConstantExprRef l,
                                      NonConstantExprRef r) {
    // A | A = A
    if (l == r)
        return l;
    ...
    if (auto ce_l = castAs<ConcatExpr>(l)) {
        if (auto ce_r = castAs<ConcatExpr>(r)) {
            if (ce_l->getLeft()->bits() == ce_r->getLeft()->bits()) {
                // right bits are same, because it is binary operation
                return createConcat(createOr(l->getLeft(), r->getLeft()),
...
ExprRef SymbolicExprBuilder::createXor(NonConstantExprRef l,
                                       NonConstantExprRef r) {
    if (l == r)
        return createConstant(0, l->bits());
    else
...
ExprRef SymbolicExprBuilder::createIte(ExprRef expr_cond, ExprRef expr_true,
                                       ExprRef expr_false) {
    if (auto BE = castAs<BoolExpr>(expr_cond))
...
ExprRef ExprBuilder::createRead(ADDRINT off) { return next_->createRead(off); }
...
ExprRef ExprBuilder::createNeg(ExprRef e) { return next_->createNeg(e); }

ExprRef ExprBuilder::createNot(ExprRef e) { return next_->createNot(e); }
...
ExprRef ExprBuilder::createLNot(ExprRef e) { return next_->createLNot(e); }

ExprRef ExprBuilder::createIte(ExprRef expr_cond, ExprRef expr_true,
                               ExprRef expr_false) {
...
ExprRef BaseExprBuilder::createIte(ExprRef expr_cond, ExprRef expr_true,
                                   ExprRef expr_false) {
...
ExprRef CacheExprBuilder::createIte(ExprRef expr_cond, ExprRef expr_true,
                                    ExprRef expr_false) {
...
ExprRef PruneExprBuilder::createIte(ExprRef expr_cond, ExprRef expr_true,
                                    ExprRef expr_false) {
