...
class RangeSet {
  ...
  protected:
    ...
    void intersectInRange(PrimRangeSet& new_ranges, const llvm::APSInt lower,
                          const llvm::APSInt upper, PrimRangeSet::iterator& i,
        ...
        //   5. R starts in the middle of the intersection range and ends after
        //   it.
  ...
  public:
    RangeSet intersect(llvm::APSInt lower, llvm::APSInt upper) {
        // Returns a set containing the values in the receiving set, intersected
        // with the closed range [Lower, Upper]. Unlike the Range type, this
        // range uses modular arithmetic, corresponding to the common treatment
        // of C integer overflow. Thus, if the Lower bound is greater than the
        // Upper bound, the range is taken to wrap around. This is equivalent to
        // taking the intersection with the two ranges [Min, Upper] and [Lower,
        // Max], or, alternatively, /removing/ all integers between Upper and
        // Lower.
        ...
        if (lower <= upper)
            ...
            // intersectInRange() does not reset the iteration state for i and
            // e. Therefore, the lower range most be handled first.
