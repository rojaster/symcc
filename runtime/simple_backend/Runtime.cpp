...
[[maybe_unused]] void dump_known_regions() {
    ...
    for (const auto& [page, shadow] : g_shadow_pages) {
        std::cerr << "  " << P(page) << " shadowed by " << P(shadow)
                  << std::endl;
    }
}
...
SymExpr registerExpression(Z3_ast expr) {
    if (allocatedExpressions.count(expr) == 0) {
        // We don't know this expression yet. Record it and increase the
        // reference counter.
...
void _sym_initialize(void) {
    ...
    initLibcWrappers();
    std::cerr
        << "This is SymCC running with the simple backend" << std::endl
        << "For anything but debugging SymCC itself, you will want to use "
           "the SymCC backend instead (see README.md for build instructions)"
        << std::endl;
...
Z3_ast _sym_build_bits_to_float(Z3_ast expr, int to_double) {
    ...
    Z3_inc_ref(g_context, (Z3_ast)sort);
    auto* result =
        registerExpression(Z3_mk_fpa_to_fp_bv(g_context, expr, sort));
    Z3_dec_ref(g_context, (Z3_ast)sort);
    return result;
}
...
void _sym_collect_garbage() {
    ...
    std::cerr << "After garbage collection: " << endSize
              << " expressions remain (before: " << startSize << ")"
              << std::endl
              << "\t(collection took "
